...
public:
	...
	*/
	...
	/**
	* brief Initializes the DMA and finds the process
	* This is required before any DMA operations can be done.
	...
	*/
	uint64_t GetBaseAddress(std::string module_name);

	/**
	* brief Gets the size of the module
	* @param module_name the name of the module
	* @return the size of the module
	*/
	uint64_t GetBaseSize(std::string module_name);
	...
	ModuleInfo GetModuleInfo(std::string module_name);

	uint64_t SigScan(uint64_t module_base, uint64_t module_size, const char* signature);
