...
bool Memory::Connect()
{
	...
	if (!this->SetFPGA())
	{
		LOG("[!] Could not set FPGA settings!\n");
		VMMDLL_Close(this->vHandle);
		this->vHandle = NULL;
		return false;
	}
...
bool Memory::FixCr3()
{
	...
	std::string lines(reinterpret_cast<char*>(bytes.get()), j);
...
Memory::ModuleInfo Memory::GetModuleInfo(std::string module_name)
{
	std::wstring str(module_name.begin(), module_name.end());
	PVMMDLL_MAP_MODULEENTRY module_info;
	if (!VMMDLL_Map_GetModuleFromNameW(this->vHandle, this->current_process.PID, (LPWSTR)str.c_str(), &module_info, VMMDLL_MODULE_FLAG_NORMAL))
	...
	return { module_info->vaBase, module_info->cbImageSize, module_name };
}

uint64_t Memory::SigScan(uint64_t module_base, uint64_t module_size, const char* signature)
{
	if (!signature || signature[0] == '\0')
		return 0;

	// Use small chunks to avoid huge allocation
	const size_t CHUNK_SIZE = 0x100000; // 1MB
	std::vector<uint8_t> buffer(CHUNK_SIZE);

	for (size_t i = 0; i < module_size; i += CHUNK_SIZE)
	{
		size_t read_size = std::min(CHUNK_SIZE, (size_t)(module_size - i));
		if (!Read(module_base + i, buffer.data(), read_size))
			continue;

		// Scan this chunk
		// We need to handle boundary cases where signature spans across chunks
		// Ideally we read (CHUNK_SIZE + MAX_SIG_LEN) but simple chunking is often enough for starters
		// Let's rely on FindSignature logic but applied to chunk, or reimplement safer logic.
		
		const char* pat = signature;
		for (size_t j = 0; j < read_size; j++)
		{
			if (*pat == '?' || buffer[j] == GetByte(pat))
			{
				if (!pat[2]) // End of signature
				{
					// Found!
					// Calculate start address: (module_base + i + j) - (sig_length_in_bytes - 1)
					// Easier: we track start of match
					// But current logic advances pat.
					
					// Re-verify match from start to be sure and get address
					// This is a quick scanner.
					
					// Let's just use a naive scanner that is correct.
					
					// Check full signature at this position
					const char* p = signature;
					size_t k = 0;
					bool match = true;
					while (*p)
					{
						if (j + k >= read_size) { match = false; break; } // Cross-chunk match not supported in this simple version
						
						if (*p != '?' && buffer[j + k] != GetByte(p))
						{
							match = false;
							break;
						}
						p += (*p == '?') ? 2 : 3;
						if (*p == ' ') p++; // skip space if any (format "AA BB")
						k++;
					}
					
					if (match)
						return module_base + i + j;
				}
