...
public:
	void Update() override
	{
		...
		if ((IsMouseInRectangle(ParentPos.x + Pos.x - 2, ParentPos.y + (Pos.y + 15) - 2, Size.x + 3, Size.y + 3) || IsMouseInRectangle(ParentPos.x + Pos.x + (Size.x * ratio), ParentPos.y + (Pos.y + 15) - (Size.y / 2.2f), 5, Size.y * 2.2f)) && IsKeyClicked(VK_LBUTTON) && !Blocked)
		{
			Held = true;
		}
		if (Held)
		{
			...
			*Value = (T)(MinValue + (MaxValue - MinValue) * ratio);
			OutputValue = *Value;
	...
	void Draw()
	{
		...
		OutlineRoundedRectangle(ParentPos.x + Pos.x + (Size.x * ratio), ParentPos.y + (Pos.y + 15) - (Size.y / 2.2f), 5, Size.y * 2.2f, 2, 2, outline);
		FilledRoundedRectangle(ParentPos.x + Pos.x + (Size.x * ratio), ParentPos.y + (Pos.y + 15) - (Size.y / 2.2f), 5, Size.y * 2.2f, 2, widget);
	}
