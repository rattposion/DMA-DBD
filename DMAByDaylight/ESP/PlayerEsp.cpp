...
void DrawPlayerEsp()
{
	...
	for (auto entity : EngineInstance->GetActors())
	{
		...
		if (screenpos == Vector2::Zero())
			continue;
		Vector3 campos = Vector3((float)EngineInstance->GetCameraCache().POV.Location.X, (float)EngineInstance->GetCameraCache().POV.Location.Y, (float)EngineInstance->GetCameraCache().POV.Location.Z);
		...
		if (distance > (float)config.MaxDistance)
			continue;
		DrawText(screenpos.x, screenpos.y, name + wdistance, "Verdana", (int)config.FontSize, config.TextColour, CentreCentre);
	}
